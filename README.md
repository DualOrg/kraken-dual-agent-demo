...
The proof and health payloads include explicit DUAL links by default. Template and object cards open the DUAL Console collection view with the exact entity id in the URL, action cards open Blockscout when an action hash is present, and every card keeps an app-served `Data` target under `/api/dual/records/...` for verified readback. Override `DUAL_CONSOLE_ORG_URL_TEMPLATE`, `DUAL_CONSOLE_TEMPLATE_URL_TEMPLATE`, `DUAL_CONSOLE_OBJECT_URL_TEMPLATE`, or `DUAL_CONSOLE_ACTION_URL_TEMPLATE` only when a more precise Console route has been verified. Blockscout defaults to `https://explorer-test-v2.dual.network`; set `DUAL_BLOCKSCOUT_BASE_URL`, `DUAL_BLOCKSCOUT_TX_URL_TEMPLATE`, or `DUAL_BLOCKSCOUT_ACTION_URL_TEMPLATE` if the network explorer changes.
